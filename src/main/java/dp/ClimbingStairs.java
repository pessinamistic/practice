 ...
 *
 * <p>Problem: You are climbing a staircase. It takes n steps to reach the top. Each time you can
...
public class ClimbingStairs {
  ...
  private static int climbStairsRecursiveHelper(int n, int depth) {
    ...
    int ways =
        climbStairsRecursiveHelper(n - 1, depth + 1) + climbStairsRecursiveHelper(n - 2, depth + 1);
  ...
  private static int climbStairsMemoHelper(int n, Integer[] memo, int depth) {
    ...
    if (memo[n] != null) {
      DPVisualizationHelper.visualizeRecursion(
          depth, "Using memoized value for ways(" + n + ") = " + memo[n]);
      return memo[n];
    }

    memo[n] =
        climbStairsMemoHelper(n - 1, memo, depth + 1)
            + climbStairsMemoHelper(n - 2, memo, depth + 1);

    DPVisualizationHelper.visualizeMemo(
        memo, n, "Updated memoization table after computing ways(" + n + ")");
    return memo[n];
  }
    ...
    for (int i = 2; i <= n; i++) {
      ...
      DPVisualizationHelper.visualize1DDP(
          ...
          "Computing dp["
              ...
              + dp[i]
              + "\n"
              + "This represents the number of ways to climb "
              + i
              + " stairs");
    }
