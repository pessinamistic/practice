 ...
 *
 * <p>Problem: You are given an integer array coins representing coins of different denominations
...
public class CoinChange {
  ...
  private static int coinChangeMemoHelper(int[] coins, int amount, Integer[] memo, int depth) {
    ...
    if (memo[amount] != null) {
      DPVisualizationHelper.visualizeRecursion(
          depth, "Using memoized value for amount " + amount + ": " + memo[amount]);
      return memo[amount];
    }
    ...
    for (int coin : coins) {
      ...
      if (result >= 0) {
        ...
        DPVisualizationHelper.visualizeRecursion(
            depth, "Found solution using coin " + coin + ", total coins: " + minCoins);
      }
    }

    memo[amount] = minCoins == Integer.MAX_VALUE ? -1 : minCoins;
    DPVisualizationHelper.visualizeMemo(
        memo, amount, "Updated memo for amount " + amount + ": " + memo[amount]);
    return memo[amount];
  }
  ...
  public static int coinChangeDP(int[] coins, int amount) {
    ...
    DPVisualizationHelper.visualize1DDP(
        ...
        "Initial DP table\ndp[0] = 0 (base case: 0 coins needed for amount 0)\n"
            + "All other values initialized to "
            + (amount + 1)
            + " (impossible value)\n"
            + "Available coins: "
  ...
  public static int coinChangeBFS(int[] coins, int amount) {
    ...
    while (front < rear) {
      ...
      System.out.println("\nProcessing amount: " + current + " (level " + levels[current] + ")");
