...
/**
 * Classic Fibonacci Problem to demonstrate Dynamic Programming concepts Problem: Find the nth
 * number in the Fibonacci sequence Fibonacci sequence: 0, 1, 1, 2, 3, 5, 8, 13, 21, ... where each
...
public class Fibonacci {
  ...
  private static long fibRecursiveWithVisualization(int n, int depth) {
    ...
    long result =
        fibRecursiveWithVisualization(n - 1, depth + 1)
  ...
  private static long fibMemoHelper(int n, Integer[] memo, int depth) {
    ...
    if (n <= 1) return n;
    if (memo[n] != null) {
      DPVisualizationHelper.visualizeRecursion(
          depth, "Using memoized value for fib(" + n + ") = " + memo[n]);
      return memo[n];
    }

    memo[n] = (int) (fibMemoHelper(n - 1, memo, depth + 1) + fibMemoHelper(n - 2, memo, depth + 1));

    DPVisualizationHelper.visualizeMemo(
        memo, n, "Updated memoization table after computing fib(" + n + ")");
    return memo[n];
  }
  ...
  public static long fibTabulation(int n) {
    ...
    for (int i = 2; i <= n; i++) {
      dp[i] = dp[i - 1] + dp[i - 2];
      DPVisualizationHelper.visualize1DDP(
          dp,
          i,
          "Computing dp["
              + i
  ...
  public static long fibOptimized(int n) {
    ...
    for (int i = 2; i <= n; i++) {
      ...
      prev1 = current;
      System.out.println(
          "Step " + i + ": current=" + current + ", prev1=" + prev1 + ", prev2=" + prev2);
    }
