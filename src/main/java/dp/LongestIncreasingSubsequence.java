 ...
 *
 * <p>Problem: Given an integer array nums, return the length of the longest strictly increasing
 * subsequence.
 */
public class LongestIncreasingSubsequence {
  ...
  private static int lengthOfLISRecursiveHelper(int[] nums, int prev, int currentPos, int depth) {
    ...
    DPVisualizationHelper.visualizeRecursion(
        depth, "At position " + currentPos + ", prev=" + prev + ", current=" + nums[currentPos]);
    ...
    if (nums[currentPos] > prev) {
      DPVisualizationHelper.visualizeRecursion(
          depth, "Including " + nums[currentPos] + " (> " + prev + ")");
        ...
        if (nums[i] > nums[j]) {
          ...
          if (newLen > dp[i]) {
            ...
            DPVisualizationHelper.visualize1DDP(
                dp,
                i,
                "Found new increasing subsequence ending at index "
                    + i
                    + "\nCurrent number "
                    + nums[i]
                    + " > "
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        "Finding Longest Increasing Subsequence in array: " + Arrays.toString(nums) + "\n");
